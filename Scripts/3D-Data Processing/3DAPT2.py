...
p_per_uniformity=[]
uniformity_values=[1.0,0.95,0.9,0.85,0.8,0.75,0.7,0.65,0.6]
uniformity_values=[0.05]
for u in uniformity_values:
    p_values=[]
    
    for cyc in range(1):# for cyc in range(cycle):
        N=50
        L=1000
        i_Ga=0
        i_In=1
    ...
    dis_from_mean=abs(p_values-mean)
    max_deviations=2
    not_outlier=dis_from_mean<max_deviations*stdev
    no_outlier=p_values[not_outlier]
    p_per_uniformity.append(np.mean(no_outlier)) 
