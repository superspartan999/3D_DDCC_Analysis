...
#
##            
#    I2=quad(lambda x :np.sqrt((2*m_p/hbar**2.0)*(f_Ec(x)-Ez)), a2, b2)[0]
#        
#
# 
#    return np.exp(-2*I2)
...
J=quad(lambda Ez: A_n*T_n(Ez)*np.log((1+np.exp(Efp-Ez)/k*300)/(1+np.exp(Efn-Ez)/k*300)), 0, Ec['Conduction Band'].iloc[-1])[0]\
     -quad(lambda Ez: A_p*T_p(Ez)*np.log((1+np.exp(Efp-Ez)/k*300)/(1+np.exp(Efn-Ez)/k*300)), 0, Ev['Valence Band'].iloc[-1])[0]
