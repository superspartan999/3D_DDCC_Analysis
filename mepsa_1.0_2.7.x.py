...
# -*- coding: utf-8 -*-
import Tkinter, Tkconstants, tkFileDialog, numpy, tkMessageBox, sys, exceptions, matplotlib, tkSimpleDialog, math, tkFont
...
def get_index_array (input_array):
    for counter in xrange (numpy.shape(input_array)[0]):
...
def create_neighbors_array(index_matrix, x_array, y_array, z_array, is_4_axes_bool):
    ...
    neighbors_array = numpy.empty((around_size+1,numpy.shape(z_array)[0])); neighbors_array.fill(-1)
    neighbor_pos = 1   
    for counter in xrange (numpy.shape(z_array)[0]):
        neighbor_pos = 1
        neighbors_array[0][counter] = 0
        for around_counter in xrange (around_size):
            ...
            if new_x >= 0 and new_x < numpy.shape(index_matrix)[0]:
                ...
                if new_y >= 0 and new_y < numpy.shape(index_matrix)[1]:
                    neighbors_array[neighbor_pos, counter] = index_matrix[new_x,new_y]
                    neighbor_pos += 1
...
def propagate_point_connectivity(working_array, point_index, z_array, neighbors_array, node_array, mask_array, first_working_point, last_pos_in_working_array, node_matrix, occupied_points_array, connectivity_propagation_array):
    for neighbor_counter in xrange (1,(neighbors_array[0][point_index]+1).astype(int)):
        if (mask_array[neighbors_array[neighbor_counter][point_index]] > 0):
            ...
            if (connectivity_propagation_array[neighbors_array[neighbor_counter][point_index]] != connectivity_propagation_array[point_index]):
    ...
    return working_array, first_working_point, last_pos_in_working_array

def define_connectivity(node_array, mask_array, z_array, neighbors_array, in_node_matrix, points_in_minima_groups):
    working_array = numpy.empty((numpy.shape(z_array)[0]+1,3)); working_array.fill(-1)#[0] index [1] previous [2] next
    ...
    connectivity_propagation_array = numpy.empty(numpy.shape(z_array)[0]); connectivity_propagation_array.fill(-1)#-1 empty, n node_number
    points_to_propagate_array = numpy.empty(numpy.shape(z_array)[0]); points_to_propagate_array.fill(-1)
    points_to_propagate_current_pos = 0
    total_points_place_holder = 0
    for counter_x in xrange (numpy.shape(points_in_minima_groups)[0]):
        for counter_y in xrange (numpy.shape(points_in_minima_groups)[1]):
    ...
    while (last_pos_in_working_array != -2):
        ...
        while (last_pos_in_working_array != -2):
            if (working_array[points_to_propagate_array[points_to_propagate_current_pos]][2] != -1 and z_array[working_array[first_working_point][0]] == z_array[working_array[working_array[points_to_propagate_array[points_to_propagate_current_pos]][2]][0]]):
                ...
                points_to_propagate_current_pos += 1
                break
        for propagate_counter in xrange (points_to_propagate_current_pos):
            working_array, first_working_point, last_pos_in_working_array = propagate_point_connectivity(working_array, working_array[points_to_propagate_array[propagate_counter]][0], z_array, neighbors_array, node_array, mask_array, first_working_point, last_pos_in_working_array, node_matrix, occupied_points_array, connectivity_propagation_array)
...
def node_list_generation (origin_point, target_point, mask_array, neighbors_array, connectivity_propagation_array):
    ...
    total_out_points = 0
    temp_pos = 0
    temp_array = numpy.empty(numpy.shape(mask_array)[0])
    temp_array[total_out_points] = target_point
    total_out_points +=1
    while mask_position != origin_point:
        temp_score = numpy.shape(mask_array)[0]
        for neighbor_counter in xrange (1,(neighbors_array[0][mask_position]+1).astype(int)):
    ...
    last_pos = 0
    for counter in xrange(total_out_points-2,-1,-1):
...
def node_finder (input_matrix, is_4_axes_bool, x_values, y_values, flat_nodes_bool):
    ...
    minima_counter = -1
    for global_counter in xrange (numpy.shape(z_array)[0]):
        if (max_check == True):
            max_check = False
        if (min_check == True):
            min_check = False
        for neighbor_counter in xrange (1,(neighbors_array[0][global_counter]+1).astype(int)):
            if (z_array[global_counter] > z_array[neighbors_array[neighbor_counter][global_counter]]):
                max_check = True
                break
    ...
    points_per_group_array = numpy.zeros(minima_groups_counter)
    ...
    max_points_in_a_group = 0
    max_points_in_a_group = 1
    points_in_minima_groups = numpy.empty((minima_groups_counter+1,max_points_in_a_group)); points_in_minima_groups.fill(-1)
    points_to_plot = numpy.empty((minima_groups_counter+1,3))
    points_to_plot_counter = 0
    for global_counter in xrange (numpy.shape(z_array)[0]):
        if (node_array[global_counter] > -1):
            points_to_plot[points_to_plot_counter][0] = x_values[x_array[global_counter]]
            points_to_plot[points_to_plot_counter][1] = y_values[y_array[global_counter]]
            points_to_plot[points_to_plot_counter][2] = node_array[global_counter]
            points_to_plot_counter += 1
            points_in_minima_groups[node_array[global_counter]][0] = global_counter
...
def insert_point_in_working_array(working_array, point_index, z_array, first_working_point, last_pos_in_working_array, occupied_points_array):
    ...
    if (first_working_point == -1):
        ...
        if (z_array[working_array[first_working_point][0]] >=  z_array[point_index]):
            ...
            while (current_pos != -1):
                if (z_array[working_array[current_pos][0]] >=  z_array[point_index]):
                    ...
                    working_array[working_array[current_pos][1]][2] = last_pos_in_working_array+1   
                ...
                if (working_array[current_pos][2] == -1):
                    ...
                    break
                current_pos = working_array[current_pos][2]
    last_pos_in_working_array += 1
    occupied_points_array[point_index] = -2
    return first_working_point, last_pos_in_working_array
    ...
    while (occupied_points_array[target_point] == 0  and last_pos_in_working_array != -2):
        ...
        while (last_pos_in_working_array != -2):
            if (working_array[points_to_propagate_array[points_to_propagate_current_pos]][2] != -1 and z_array[working_array[first_working_point][0]] == z_array[working_array[working_array[points_to_propagate_array[points_to_propagate_current_pos]][2]][0]]):
                ...
                break
        for propagate_counter in xrange (points_to_propagate_current_pos):
            total_points, working_array, first_working_point, last_pos_in_working_array = propagate_point(working_array, working_array[points_to_propagate_array[propagate_counter]][0], z_array, neighbors_array, mask_array, first_working_point, last_pos_in_working_array, occupied_points_array, total_points)
...
def get_points (origin_point, target_point, mask_array, neighbors_array, x_values, y_values, x_array, y_array, z_array):
    ...
    for counter in xrange(total_out_points):
        out_points[out_pos][0] = x_values[x_array[temp_array[counter]]]
        out_points[out_pos][1] = y_values[y_array[temp_array[counter]]]
...
def connectivity_propagation_array_analysis(connectivity_propagation_array, points_in_minima_groups, x_values, y_values, x_array, y_array, z_array, neighbors_array):
    total_points_array = numpy.zeros(numpy.shape(points_in_minima_groups)[0])
    minimum_barrier_matrix = numpy.empty((numpy.shape(points_in_minima_groups)[0],numpy.shape(points_in_minima_groups)[0])); minimum_barrier_matrix.fill(-1)
    for counter in xrange(numpy.shape(connectivity_propagation_array)[0]):
        total_points_array[connectivity_propagation_array[counter]] += 1
        for neighbor_counter in xrange (1,(neighbors_array[0][counter]+1).astype(int)):
            if connectivity_propagation_array[counter] != connectivity_propagation_array[neighbors_array[neighbor_counter][counter]]:
    ...
    total_barriers = 0    
    for x in xrange(numpy.shape(points_in_minima_groups)[0]):
        for y in xrange(x,numpy.shape(points_in_minima_groups)[0]):
            if minimum_barrier_matrix[x][y] != -1:
                temp_pos = minimum_barrier_matrix[x][y]
                minimum_barrier_matrix[x][y] = -1
                for neighbor_counter in xrange (1,(neighbors_array[0][temp_pos]+1).astype(int)):
...
def global_connectivity_analyis (input_matrix, node_array, points_in_minima_groups, node_matrix_template, is_4_axes_bool, x_values, y_values):
    ...
    connectivity_propagation_array, connectivity_node_matrix = define_connectivity (node_array, mask_array, z_array, neighbors_array, node_matrix_template, points_in_minima_groups)
    ...
    for map_counter in xrange(numpy.shape(connectivity_propagation_array)[0]):
        if connectivity_propagation_array[map_counter] == counter:
            temp_points[out_pos][0] = x_values[x_array[map_counter]]
            temp_points[out_pos][1] = y_values[y_array[map_counter]]
            temp_points[out_pos][2] = connectivity_propagation_array[map_counter]
            out_pos += 1
    points_list = temp_points.tolist()
    points_stack = [points_list,]
    for counter in xrange(1,numpy.shape(points_in_minima_groups)[0]):
        temp_points = numpy.empty((total_points_array[counter],3))
        out_pos = 0
        for map_counter in xrange(numpy.shape(connectivity_propagation_array)[0]):
            if connectivity_propagation_array[map_counter] == counter:
    ...
    out_pos = 0
    for x in xrange(numpy.shape(points_in_minima_groups)[0]):
        minima_points[x][0] = x_values[x_array[points_in_minima_groups[x][0]]]
        minima_points[x][1] = y_values[y_array[points_in_minima_groups[x][0]]]
        minima_points[x][2] = z_array[points_in_minima_groups[x][0]]
        for y in xrange(x,numpy.shape(points_in_minima_groups)[0]):
            if minimum_barrier_matrix[x][y] != -1:
                barrier_points[out_pos][0] = x_values[x_array[minimum_barrier_matrix[x][y]]]
                barrier_points[out_pos][1] = y_values[y_array[minimum_barrier_matrix[x][y]]]
...
def find_path (input_matrix, origin_point, target_point, node_array, points_in_minima_groups, node_matrix_template, is_4_axes_bool, x_values, y_values, if_node_by_node):
    ...
    if if_node_by_node == 0:
        ...
        for counter in xrange (2, numpy.shape(final_node_list)[0]):
            print str(counter) + " of " + str(numpy.shape(final_node_list)[0]-1)
            mask_gen_bool, temp_mask_array, total_points = mask_gen (points_in_minima_groups[final_node_list[counter-1]][0], points_in_minima_groups[final_node_list[counter]][0], OT_mask_array, z_array, neighbors_array)
    ...
    for counter in xrange (numpy.shape(OT_mask_array)[0]):
        if (OT_mask_array[counter] > 0):
            visited_points[visited_points_position][0] = x_values[x_array[counter]]
            visited_points[visited_points_position][1] = y_values[y_array[counter]]
...
def smooth_path(path_points, iterations):
    ...
    for counter in xrange(numpy.shape(path_points)[0]):
    ...
    writing_layer = 0
    reference_layer = 1
    for iteration_counter in xrange(iterations):
        writing_layer, reference_layer = reference_layer, writing_layer
        for counter in xrange(1,numpy.shape(path_points)[0]-1):
...
def smooth_map(input_matrix, is_4_axes_bool, iterations):
    ...
    for counter in xrange(numpy.shape(x_array)[0]):
    ...
    for iteration_counter in xrange(iterations):
        ...
        for counter in xrange(numpy.shape(x_array)[0]):
            ...
            for neighbor_counter in xrange (1,(neighbors_array[0][counter]+1).astype(int)):
                map_smooth_temp[writing_layer][counter] += map_smooth_temp[reference_layer][neighbors_array[neighbor_counter][counter]]
            map_smooth_temp[writing_layer][counter] = map_smooth_temp[writing_layer][counter] / (neighbors_array[0][counter]+1).astype(int)
        print_progress (iteration_counter,iterations)
    for counter in xrange(numpy.shape(x_array)[0]):
        out_matrix[0][x_array[counter]][y_array[counter]] = map_smooth_temp[writing_layer][counter]
...
def create_index_matrix(reference_matrix, x_array, y_array):
    ...
    for counter in xrange (numpy.shape(x_array)[0]):
        index_matrix[x_array[counter]][y_array[counter]] = counter
    return index_matrix
...
def node_search(x_array, y_array, z_array, node_array, x_values, y_values, x_center, x_increment, y_center, y_increment):
    ...
    for counter in xrange(numpy.shape(z_array)[0]):
        if node_array[counter] > -1:
            if x_values[x_array[counter]] >= x_range[0] and x_values[x_array[counter]] <= x_range[1]:
                if y_values[y_array[counter]] >= y_range[0] and y_values[y_array[counter]] <= y_range[1]:
                    if temp_counter == -1:
...
def node_arbitrary(x_array, y_array, x_values, y_values, x_center, y_center):
    shortest_distance = distance(x_center, y_center, x_values[x_array[0]], y_values[y_array[0]])
    temp_counter = 0
    for counter in xrange(1, numpy.shape(x_array)[0]):
        current_distance = distance(x_center, y_center, x_values[x_array[counter]], y_values[y_array[counter]])
...
def find_OT(input_matrix, node_array, x_values, y_values, x_origin_center, y_origin_center, x_origin_increment, y_origin_increment, x_target_center, y_target_center, x_target_increment, y_target_increment, origin_menu_var, target_menu_var, points_in_minima_groups):
    ...
    if origin_temp_counter != -1:
        origin_node_to_plot[0] = x_values[x_array[origin_temp_counter]]
        origin_node_to_plot[1] = y_values[y_array[origin_temp_counter]]
        origin_node_to_plot[2] = z_array[origin_temp_counter]
    if target_temp_counter != -1:
        target_node_to_plot[0] = x_values[x_array[target_temp_counter]]
        target_node_to_plot[1] = y_values[y_array[target_temp_counter]]
...
def rectangle_stamp(input_matrix, sorted_x_axis, sorted_y_axis, center_x, center_y, x_increment, y_increment, value_to_add):
    ...
    if value_to_add != 0:
        ...
        for x in xrange(numpy.shape(sorted_x_axis)[0]):
            if sorted_x_axis[x]<=x_up_limit and sorted_x_axis[x]>=x_down_limit:
                for y in xrange(numpy.shape(sorted_y_axis)[0]):
...
def surface_plot(xi, yi, x_to_plot, y_to_plot, z_to_plot):
    ...
    try:
        ...
        pass  
    plt.figure(1)
    nx, ny = numpy.shape(xi)[0], numpy.shape(yi)[0]
    xmax, xmin = max(xi), min(xi)
    ymax, ymin = max(yi), min(yi)
    zi = ml.griddata(x_to_plot, y_to_plot, z_to_plot, xi, yi)
    ...
    if data_type == "list":
        ...
        for arg in args:
            ...
            for points_list in arg:
                ...
                for counter in xrange(numpy.shape(temp_arg)[0]):
                    point_counter += 1
                    print_progress (point_counter,numpy.shape(temp_arg)[0])
                    plt.scatter(temp_arg[counter][0],temp_arg[counter][1],color = temp_color, alpha=0.5)
                color_counter += 1
                print "Done"
        ...
        for arg in args:
            ...
            if len(numpy.shape(arg)) == 2:
                for counter in xrange(numpy.shape(arg)[0]):
                    point_counter += 1
        ...
        for arg in args:
            ...
            if len(numpy.shape(arg)) == 2:
                for counter in xrange(numpy.shape(arg)[0]):
                    point_counter += 1
                    print_progress (point_counter,numpy.shape(arg)[0])
                    plt.scatter(arg[counter][0],arg[counter][1],color = temp_color, alpha=0.5)
            elif len(numpy.shape(arg)) == 1:
                point_counter += 1
                print_progress (point_counter,numpy.shape(arg)[0])
...
def generate_three_column_format(sorted_x_array, sorted_y_array, z_matrix):
    ...
    counter_to_plot = 0
    for counter_x in xrange(0, sorted_x_length[0]):
       for counter_y in xrange (0, sorted_y_length[0]):
...
def sort_and_clean(in_array):
    ...
    sort_index[0]=True
    for counter_1 in xrange(1, array_length[0]):
        if sorted_array[counter_1] != sorted_array[counter_1-1]:
            sort_index[counter_1]=True
    out_array = []
    for counter_1 in xrange(0, array_length[0]):
...
def three_dimension_sorter(raw_x_array, raw_y_array, raw_z_array):
    ...
    if raw_x_length[0] <= 0 or raw_x_length[0] != raw_y_length[0] or raw_x_length[0] != raw_z_length[0]:
        ...
        for counter_1 in xrange(0, raw_z_length[0]):
            out_matrix[0][x_index_array[counter_1]][y_index_array[counter_1]] = raw_z_array[counter_1]  
        return sorted_x, sorted_y, out_matrix
        
def sampling_filter_iterator(visited_points, area_to_ignore):
    filtered_points_temp = numpy.empty(numpy.shape(visited_points))
    insert_counter = 0
    filter_check = True
    for points_counter in xrange(numpy.shape(visited_points)[0]):
        if (filter_check == False):
            filter_check = True
        for area_to_ignore_counter in xrange(numpy.shape(area_to_ignore)[0]):
            if (area_to_ignore[area_to_ignore_counter][0] == visited_points[points_counter][0] and area_to_ignore[area_to_ignore_counter][1] == visited_points[points_counter][1]):
                filter_check = False
                break
        if (filter_check == True):
            filtered_points_temp[insert_counter][0] = visited_points[points_counter][0]
            filtered_points_temp[insert_counter][1] = visited_points[points_counter][1]
            filtered_points_temp[insert_counter][2] = visited_points[points_counter][2]
            insert_counter += 1
    filtered_points_out = numpy.empty((insert_counter,3))
    for points_counter in xrange(insert_counter):
        filtered_points_out[points_counter][0] = filtered_points_temp[points_counter][0]
        filtered_points_out[points_counter][1] = filtered_points_temp[points_counter][1]
        filtered_points_out[points_counter][2] = filtered_points_temp[points_counter][2]
    return filtered_points_out
    
def sampling_points_filter(origin_visited_points, target_visited_points, origin_area, target_area):
    filtered_origin_points = sampling_filter_iterator (origin_visited_points, target_area)
    filtered_target_points = sampling_filter_iterator (target_visited_points, origin_area)
    return filtered_origin_points, filtered_target_points
...
class TkMainDialog(Tkinter.Frame):
    ...
    def __init__(self, root):
        ...
        self.mepsa_label.pack(fill = Tkconstants.BOTH, padx = 10, pady = 10)
...
####################### Map editor
    ...
    def map_editor(self):
        ...
        def callback_save_map():
            file_to_save = tkFileDialog.asksaveasfile(mode='w', defaultextension=".txt")
            if file_to_save is None == 0:
                return
            path_counter = 0
            for x in xrange(numpy.shape(self.maps_list[numpy.shape(self.maps_list)[0]-1])[0]):
                for y in xrange(numpy.shape(self.maps_list[numpy.shape(self.maps_list)[0]-1])[1]):
...
####################### Connectivity analyses
    ...
    def path_generator(self):
        ...
        def check_status():
            ...
            if self.path_not_found == 1 or self.global_connectivity_not_found == 1 or self.origin_selection_method == "ARBITRARY" or self.target_selection_method == "ARBITRARY":
        ...
        def callback_analyse_global_connectivity():
        ...
        def callback_find_nodes(mode):
            ...
            if (self.nodes_not_found != 0):
                ...
                for counter in xrange(numpy.shape(self.points_in_minima_groups)[0]):
        ...
        def callback_find_interest_points():
            temp_interest_points = numpy.empty(numpy.shape(self.path_points)[0]);
            max_check = 0
            min_check = 0
            total_interest_points = 1
            temp_interest_points[0] = 0
            for counter in xrange(1,numpy.shape(self.path_points)[0]-1):
                max_check = 0
                min_check = 0
            ...
            for counter in xrange (total_interest_points):
        ...
        def callback_path_line_plot():
            ...
            for counter in xrange(numpy.shape(self.path_points)[0]):
        ...
        def callback_save_interest_points():
            ...
            if file_to_save is None == 0:
                return
            for counter in xrange(numpy.shape(self.interest_points)[0]):
        ...
        def callback_save_smooth():
            ...
            if file_to_save is None == 0:
                return
            for counter in xrange(numpy.shape(self.smooth_path_points)[0]):
        ...
        def callback_save_stack():
            ...
            for points_list in self.path_points_stack:
                ...
                path_counter+=1
                for counter in xrange(numpy.shape(temp_array)[0]):
...
##################################################
        def callback_calculate_well_sampling():
            ...
            origin_area = self.connectivity_points_stack[int(self.connectivity_propagation_array[int(self.origin_point)])]
            target_area = self.connectivity_points_stack[int(self.connectivity_propagation_array[int(self.target_point)])]
            temp_path_not_found, temp_path_points, target_visited_points = find_path(self.maps_list[numpy.shape(self.maps_list)[0]-1], self.target_point, self.origin_point, self.node_array, self.points_in_minima_groups, self.node_matrix, self.is_4_axes_bool, self.x, self.y, 0)

            self.origin_sampling_points, self.target_sampling_points = sampling_points_filter(self.visited_points, target_visited_points, origin_area, target_area)
        ...
        def callback_well_sampling_plot():
            ...
            x_to_plot, y_to_plot, z_to_plot = generate_three_column_format(self.x, self.y, self.maps_list[numpy.shape(self.maps_list)[0]-1])
            surface_plot(self.x, self.y, x_to_plot, y_to_plot, z_to_plot)
            add_points_to_surface_plot(self.origin_sampling_points, self.target_sampling_points, color = ["#00ff00", "black"])
        ...
        def callback_save_well_sampling():
            x_to_plot, y_to_plot, z_to_plot = generate_three_column_format(self.x, self.y, self.maps_list[numpy.shape(self.maps_list)[0]-1])
            file_to_save = tkFileDialog.asksaveasfile(mode='w', defaultextension=".tx   t")
            if file_to_save is None == 0:
                return
            path_counter = 0
            file_to_save.write("TARGET_TO_ORIGIN\n")
            for counter in xrange(numpy.shape(self.origin_sampling_points)[0]):
            ...
            for counter in xrange(numpy.shape(self.target_sampling_points)[0]):
